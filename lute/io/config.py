...
def parse_config(task_name: str = "test", config_path: str = "") -> TaskParameters:
    ...
    if version_specifier is not None and version_specifier > 0:
        ...
        # We don't need the version_location or diff args info anymore
        if hasattr(parsed_parameters.Config, "version_location"):
            del parsed_parameters.Config.version_location

        if hasattr(parsed_parameters.Config, "version_diff_args"):
            del parsed_parameters.Config.version_diff_args
