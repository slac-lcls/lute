...
from lute.io.models.geom_opt import OptimizePyFAIGeomParameters
...
class OptimizePyFAIGeom(Task):
    """Optimize detector geometry using PyFAI coupled with Bayesian Optimization."""

    def __init__(self, *, params: OptimizePyFAIGeomParameters, use_mpi: bool = True) -> None:
        super().__init__(params=params, use_mpi=use_mpi)

    def _run(self) -> None:
        detector = self.build_pyFAI_detector()
        optimizer = self.BayesGeomOpt(
            exp=self.params.exp,
            run=self.params.run,
    ...
    def update_geometry(self, optimizer):
        ...
        """
        PyFAIToCrystFEL(detector=optimizer.detector, params=optimizer.params, psana_file=self.params.in_file, out_file=self.params.out_file.replace("0-end.data", f"r{optimizer.run:04}.geom"))
        CrystFELToPsana(in_file=self.params.out_file.replace("0-end.data", f"r{optimizer.run:04}.geom"), det_type=optimizer.det_type, out_file=self.params.out_file)
